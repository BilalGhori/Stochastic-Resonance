...
params.addParameter('patchSaturation', 0.05, @(x) isnumeric(x) && x>=0 && x<=1);
params.addParameter('LineWidth', 4, @(x) isnumeric(x) && x>=0);
...
function H = makePlot(x,y,errBar,lineProps,transparent,patchSaturation,LineWidth)
    ...
    set(H.patch,'facecolor',patchColor,'edgecolor','none','facealpha',faceAlpha)
